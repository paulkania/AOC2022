...
with open('12r.txt','r') as input: ###CHANGE START_POS AND END_POS!!!
...
start_position = [20,0] # [20,0] for r !!!
end_position = [154,20] #5R 2Down. For R: [154,20] !!!
...
visit_dict[(0,20)]= True #!!! change for R
...
while Q:
    ...
    if colp-1 >= 0 and visit_dict[(rowp,colp-1)] == False: # look LEFT? 
        if (ord(input[rowp][colp-1]) - cp_score  < 2):
                if [rowp,colp-1] == [20,154]:
                    print(score,'score')
                else:
                    appended +=1
                    grid[rowp][colp-1] = grid[rowp][colp] +1
